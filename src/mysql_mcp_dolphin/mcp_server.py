...
from mysql_mcp_dolphin.config import load_config, PERMISSIONS
from mysql_mcp_dolphin.db_conn import MySQLConnectionPool, logger

config = load_config()
role = config.pop("role", "r")
dolphinMcp = FastMCP("dolphin MCP")
...
async def _execute_single_sql(sql: str):
    """执行sql"""
    results = []
    column_comment = {}
...
@dolphinMcp.tool
async def analyze_table_status(table_name: str) -> str:
    """
...
async def mcp_run(mode='stdio'):
    try:
        ...
        if mode == 'sh':
            await dolphinMcp.run_async(transport="streamable-http", host="0.0.0.0", port=9009)
        elif mode == 'sse':
            await dolphinMcp.run_async(transport="sse", host="0.0.0.0", port=9009)
        else:
            await dolphinMcp.run_async(transport="stdio")
    except Exception as e:
