...
## 项目简介
MySQL MCP Dolphin 是一个基于 FastMCP 的 MySQL MCP Server项目，提供了一个安全、高效的接口来执行 SQL 操作。该项目支持多种权限控制（读、写、管理员），并通过工具函数实现了表结构查询、索引信息获取、健康状态监控等功能。
...
## 快速开始
1. 安装：`pip install mysql-mcp-dolphin`
...
```
3. 启动命令：
   - STDIO：uv run -m mysql_mcp_dolphin.mcp_server 
   - Streamable HTTP： uv run -m mysql_mcp_dolphin.mcp_server sh
   - SSE: uv run -m mysql_mcp_dolphin.mcp_server sse
...
在 MCP 配置文件中添加如下配置，以使用 `uvx` 部署 MySQL MCP Dolphin 服务：
#### STDIO
...
```json
{
  "mcpServers": {
    "mysql_mcp_dolphin": {
      ...
      "args": [
        "--directory",
        "D:/mysql_mcp_dolphin/src/mysql_mcp_dolphin",
        "run",
        "-m",
        "mcp_server"
      ]
...
```json
{
  "mcpServers": {
    "mysql_mcp_dolphin": {
      ...
      "args": [
        "--directory",
        "D:/mysql_mcp_dolphin/src/mysql_mcp_dolphinu",
        "run",
        "-m",
        "mcp_server",
        "sse"
